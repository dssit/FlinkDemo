...
object FlinkStreamWordCount {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //3、读取数据,读取sock流中的数据 ,这个hostname写本机的，在linux环境中输入hostname就可以拿到它的值。
    val stream: DataStream[String] = streamEnv.socketTextStream(hostname,port) //DataStream ==> spark 中Dstream
